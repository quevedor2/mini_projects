...
getMotif <- function(p, pfm=NULL, bsgenome=NULL, motif=NULL,motif_tolerance=200){
  # Top scoring motif per position 
  if(!is.null(motif)){
    start(p@anno) <- start(p@anno)-motif_tolerance
    end(p@anno) <- end(p@anno)+motif_tolerance
    psort <- sort(p@anno)
    ov <- findOverlaps(psort, motif)
...
key <- 'ENSEMBL'  # 'REFSEQ', 'ENSEMBL'
txby <- keys(genome, key)
gene_ids <- mapIds(genome, keys=txby, column='SYMBOL',
                   keytype=key, multiVals="first")
# names(gene_ids) <- gsub("MUSG", "G", names(gene_ids))
...
peaks <- lapply(file_list_split, function(files_df){
  ...
  grl <- as(lapply(files, function(f){
    df <- read.table(f, header = T, stringsAsFactors = F, check.names = F, sep="\t")
    colnames(df) <- gsub("L28_L4", "L28_3", colnames(df))
    gr0 <- makeGRangesFromDataFrame(df, keep.extra.columns = T)
    seqlevelsStyle(gr0) <- 'NCBI'
    gr0
  }), 'GRangesList')
  names(grl) <- gsub("^.*ATAC_(.+)_normalized.*$", "\\1", files_df$file)
  
  ## Categorize the peaks based on their genomic annotation (CEAS style plots)
  peakAnnoList <- lapply(grl, annotatePeak, TxDb=txdb, level = "gene",
                         tssRegion=c(-3000, 3000), verbose=FALSE)
  for(idx in seq_along(peakAnnoList)){
    # peakAnnoList[[idx]]@anno$symbol2 <- gene_ids[peakAnnoList[[idx]]@anno$refseqID]
...
peaks_goi <- lapply(unlist(peaks, recursive=F), function(p){
  ...
  dp_goi <- dp[unlist(sapply(paste0("^", mouse_goi, "$"), 
...
x <- lapply(names(peaks_goi), function(id){
  if(!file.exists(file.path(outdir, paste0(id, "-targetGenes.tsv")))){
    ...
    write.table(df[,colids], file=file.path(outdir, "mouse", paste0(id, "-targetGenes.tsv")),
...
dds <- DESeq(dds)
res <- results(dds)
res_N_L28 <- lfcShrink(dds, coef="grp_N_vs_L28", type="apeglm")
res_L7_L28 <- lfcShrink(dds, coef="grp_L7_vs_L28", type="apeglm")
resl <- list("N-L28"=res_N_L28,
             "L7-L28"=res_L7_L28)

dds <- DESeqDataSetFromMatrix(countData=peaks_heights_mat[,-grep("^L7", colnames(peaks_heights_mat))], 
                              colData=peaks_meta[-grep("L7", peaks_meta$grp),], 
                              design=~grp, tidy = TRUE)
dds <- DESeq(dds)
...
gettop <- TRUE
ggps <- lapply(names(resl), function(compgrp){
  res <- resl[[compgrp]]
  ...
  #select most significant and highest fold change
  if(gettop){
    ...
    resdf$sig <- setNames(c('q<0.0001 & log2LFC>>2', ''), c('TRUE', 'FALSE'))[as.character(resdf$sig)]
  }
  ...
  write.table(resdf, file=file.path(outdir, "mouse", paste0("DESeq2_dag.", compgrp, ".tsv")),
...
pdf(file.path(outdir, "mouse", "volcano_plot.pdf"), width=13)
# pdf(file.path("~/xfer", "volcano_plot.pdf"), width=13)
plot_grid(plotlist = ggps, ncol=2)
dev.off()

# pdf(file.path(outdir, "mouse", "tox_volcano.pdf"))
# gp
# dev.off()
...
motif_gs[grep("Tox", motif_gs$gene),]
motif_gs[grep("4:69", motif_gs$gene),]
...
lfc_l <- lapply(names(resl), function(compgrp){
  res <- resl[[compgrp]]
  res_gr <- makeGRangesFromDataFrame(
    data.frame(chr=gsub(".*_([0-9XYMT]*).*", "\\1", rownames(res)),
  ...
  write.table(irf2_lfc_sig, file=file.path(outdir, "mouse", paste0("sig_irf2_genes.", compgrp, ".tsv")),
              col.names=T, row.names = T, sep="\t", quote = F)
  return(list("lfc"=lfc_v, "p"=padj_v))
})
names(lfc_l) <- names(resl)
