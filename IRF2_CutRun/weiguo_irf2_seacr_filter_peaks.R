...
# Generate Cut&Run peak catalogue - based off of Unfixed IRF2 and IRF2-44 samples
...
#######################################
#### 6. Visualize motif enrichment ####
## Lollipop plot of HOMER motif enrichment
homer_f <- '/cluster/projects/mcgahalab/data/mcgahalab/sabelo_irf2/weiguo_cutandtag_july29/results/manual/filtered/homerPromoter_AC_uF_IRF2-44/knownResults.txt'
homer <- read.table(homer_f, sep="\t", header=T, stringsAsFactors = F, 
                    check.names = F, comment.char = "")
melt_homer <- homer %>% 
  mutate(Motif=gsub("\\(.*", "", `Motif Name`),
         logP=(-1*`Log P-value`),
         Percent_on_target=as.numeric(gsub("%", "", `% of Target Sequences with Motif`)),
         Percent_on_background=as.numeric(gsub("%", "", `% of Background Sequences with Motif`)),
         Target_to_background=Percent_on_target/Percent_on_background) %>%
  filter(`q-value (Benjamini)` < 0.05) %>%
  select(Motif, logP, Percent_on_target, Percent_on_background, Target_to_background)
melt_homer$Motif <- factor(melt_homer$Motif, levels=melt_homer$Motif)

pdf("~/xfer/motif_enrichment.pdf", height = 6, width = 8)
ggplot(melt_homer[1:50,], aes(x=Motif, y=logP, size=Target_to_background)) +
  geom_segment(aes(x = Motif, xend = Motif, y = 0, yend = logP, size=1)) +
  geom_point() + 
  ylab("-log(p)") + xlab("Motifs") + ylim(0,450) +
  scale_size(range = c(0.5, 4)) +
  theme_classic() +
  theme(axis.text.x=element_text(angle=90, hjust = 1))
dev.off()
...
#################################
#### 6. QC Peak distribution ####
## Get Motif data from JASPAR2020
library(JASPAR2020)
library(TFBSTools)


genesets <- grep("hallmark_xenobiotic|reactome_tryptophan|reactome_metabolism_of_amino|GOBP_tryptophan_.*bolic", 
     all_gene_sets$gs_name, ignore.case = T, value = T) %>%
  sort %>% unique
...
id='AC_uF_IRF2-44'
cnr_irf2 <- unlist(as(grs_anno[[id]], 'GRangesList'))
cnr_irf2_raw <- cnr_catalogue_bkup$`AC_uF_IRF2-44.stringent.bed`
ov_idx <- findOverlaps(cnr_irf2_raw, cnr_irf2)


# Get the Tag Matrix for deepmap visualization
seqlevelsStyle(cnr_irf2) <- 'ENSEMBL'
tagmatrix <- getTagMatrix(cnr_irf2, TxDb = txdb,
                          upstream=3000, downstream=3000, type='start_site',
                          by='gene', nbin=800)
pdf("~/xfer/tag2.pdf")
# plotAvgProf2(peak=cnr_irf2, TxDb=txdb, upstream=3000, downstream=3000, 
#              conf = 0.95, resample = 1000)
plotPeakProf2(peak = cnr_irf2, upstream = rel(0.2), downstream = rel(0.2),
              conf = 0.95, by = "gene", type = "body", nbin = 800,
              TxDb = txdb, ignore_strand = F)

names(cnr_irf2) <- NULL
anno_obj <- annotatePeak(peak=cnr_irf2, TxDb=txdb, level='gene',
             tssRegion=c(-3000, 3000), verbose=FALSE)
pdf("~/xfer/tag3.pdf", height = 3)
plotAnnoBar(anno_obj)
# tagHeatmap(list(tagmatrix), xlim=c(-3000, 3000), color='#636363')
dev.off()
