...
outdir <- 'output/NatCan_2019'
setwd(PDIR)
dir.create(outdir, recursive = TRUE, showWarnings = FALSE)

meta_bcc <- read.table(file.path("data/scRNA/NatCan_2019", "GSE123813_bcc_all_metadata.txt.gz"),
                   header=TRUE, sep="\t", stringsAsFactors = F, check.names = F)
meta_scc <- read.table(file.path("data/scRNA/NatCan_2019", "GSE123813_scc_metadata.txt.gz"),
                       header=TRUE, sep="\t", stringsAsFactors = F, check.names = F)
cnts_bcc <- read.table(file.path("data/scRNA/NatCan_2019", "GSE123813_bcc_scRNA_counts.txt.gz"),
                   header=TRUE, sep="\t", stringsAsFactors = F, check.names = F)
cnts_scc <- read.table(file.path("data/scRNA/NatCan_2019", "GSE123813_scc_scRNA_counts.txt.gz"),
                   header=TRUE, sep="\t", stringsAsFactors = F, check.names = F)
...
seu_bcc <- CreateSeuratObject(counts = cnts_bcc, min.cells = 3, min.genes = 200, assay = "RNA", project = "BCC")
meta_bcc_tmp <- meta_bcc[match(rownames(seu_bcc@meta.data), meta_bcc$cell.id),]
seu_bcc@meta.data <- cbind(seu_bcc@meta.data, meta_bcc_tmp)
seu_scc <- CreateSeuratObject(counts = cnts_scc, min.cells = 3, min.genes = 200, assay = "RNA", project = "BCC")
meta_scc_tmp <- meta_scc[match(rownames(seu_scc@meta.data), meta_scc$cell.id),]
seu_scc@meta.data <- cbind(seu_scc@meta.data, meta_scc_tmp)
seu <- merge(seu_bcc, y = seu_scc, add.cell.ids = c("bcc", "scc"), project = "NatCan_2019")
SeuratDisk::SaveH5Seurat(seu, filename = file.path(outdir, "seurat_obj", paste0("natcan2019.h5seurat")), 
                         overwrite = TRUE)
rm(seu_scc, seu_bcc, cnts_bcc, cnts_scc)
...
seu_filt <- ScaleData(object = seu_filt, vars.to.regress = c("CC.Difference", "percent.mt"))
...
SeuratDisk::SaveH5Seurat(seu_harmony, filename = file.path(outdir, "seurat_obj", 
                                                           paste0("natcan2019_harmony.h5seurat")), overwrite = TRUE)

#############################
#### 3. Marker Detection ####
seu_harmony <- SeuratDisk::LoadH5Seurat(file.path(outdir, "seurat_obj", "natcan2019_harmony.h5seurat"))
seu_harmony_natcan <- seu_harmony
seu_harmony_natcan@reductions$umap@cell.embeddings[,1] <- seu_harmony$UMAP1
seu_harmony_natcan@reductions$umap@cell.embeddings[,2] <- seu_harmony$UMAP2

datatype <- 'Gene_Expression' # 'Gene_Expression', 'NatCan'
if(!file.exists(file.path(outdir, "harmony", paste0(datatype,"_markers.rda")))){
  Idents(seu_harmony) <- if(datatype=='NatCan') seu_harmony$cluster else seu_harmony$seurat_clusters
  ...
  write.table(seu_harmony_markers, file=file.path(outdir, "harmony", paste0(datatype, ".seurat_markers.csv")),
...
nL <- R.utils::countLines(file.path(outdir, "harmony", paste0(datatype, '.annotate.txt')))
df <- read.csv(file.path(outdir, "harmony", paste0(datatype, '.annotate.txt')), header=FALSE, skip=nL-cluster_keep,
...
if(!file.exists(file.path(outdir, "annotation", paste0("hpca.rds")))){
  #hpca.se <- HumanPrimaryCellAtlasData()
  #saveRDS(hpca.se, file="~/downloads/HumanPrimaryCellAtlasData.rds")
  hpca.se <- readRDS("/cluster/projects/mcgahalab/ref/scrna/scRNAseq_datasets/HumanPrimaryCellAtlasData.rds") 
  pred.seu <- SingleR(test=GetAssayData(seu_harmony), ref=hpca.se, 
                      assay.type.test=1, labels=hpca.se$label.fine)
  saveRDS(pred.seu, file=file.path(outdir, "annotation", paste0("hpca.rds")))
} else {
  hpca_anno <- readRDS(file.path(outdir, "annotation", paste0("hpca.rds")))
}


#out <- listDatasets()
#saveRDS(sceM, file="~/downloads/KotliarovPBMCData.rds")
#sceM <- readRDS("/cluster/projects/mcgahalab/ref/scrna/scRNAseq_datasets/MairPBMCData.rds") 
# sceM <- logNormCounts(sceM)
# pred.hesc <- SingleR(test = GetAssayData(seu_harmony), ref = hpca.se, assay.type.test=1,
...
plot_grid(meltAnno(split(seu_harmony$bp.fine.cluster, f=seu_harmony$treatment)),
          meltAnno(split(seu_harmony$bp.fine.cell, f=seu_harmony$treatment)),
          meltAnno(split(seu_harmony$cluster, f=seu_harmony$treatment)),
          nrow=3, ncol=1)

plot_grid(meltAnno(split(seu_st2pos$bp.fine.cluster, f=seu_st2pos$treatment)),
          meltAnno(split(seu_st2pos$bp.fine.cell, f=seu_st2pos$treatment)),
          meltAnno(split(seu_st2pos$cluster, f=seu_st2pos$treatment)),
          nrow=3, ncol=1)

plot_grid(meltAnno(split(seu_st2neg$bp.fine.cluster, f=seu_st2neg$treatment)),
          meltAnno(split(seu_st2neg$bp.fine.cell, f=seu_st2neg$treatment)),
          meltAnno(split(seu_st2neg$cluster, f=seu_st2neg$treatment)),
          nrow=3, ncol=1)
dev.off()
...
p_lbl <- DimPlot(object = seu_harmony, reduction = "umap", group.by='cluster', 
              label = TRUE, pt.size = 0.5)
...
p_bpfc <- DimPlot(object = seu_harmony, reduction = "umap", group.by='bp.fine.cell', 
                  label = TRUE, pt.size = 0.5, shuffle = TRUE)
p_tx <- DimPlot(object = seu_harmony, reduction = "umap", group.by='treatment', 
              label = TRUE, pt.size = 0.5, shuffle = TRUE)
p_cl + p_lbl
p_lbl + p_scsa
p_lbl + p_bpfl
...
Idents(seu_harmony) <- seu_harmony$cluster
...
clusters <- c('cluster', 'bp.fine.cell', 'bp.fine.cluster')
for(cl in clusters){
  print(paste0(cl, "..."))
