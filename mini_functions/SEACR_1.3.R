...
# Manual
if(1==0){
  setwd("/cluster/projects/mcgahalab/data/mcgahalab/sabelo_irf2/weiguo_cutandtag/results/peaks/seacr_manual/Act_unfixed_H3K4me3_Act_unfixed_IgG_relaxed")
  argsL <- list("exp"="Act_unfixed_H3K4me3.auc", "ctrl"="Act_unfixed_IgG.auc",
                "norm"="no", "output"="Act_unfixed_H3K4me3")
  
  setwd("/cluster/projects/mcgahalab/data/mcgahalab/sabelo_irf2/weiguo_cutandtag/results/peaks/seacr_manual/Act_unfixed_IRF2_Act_unfixed_IgG_relaxed")
  argsL <- list("exp"="Act_unfixed_IRF2.auc", "ctrl"="Act_unfixed_IgG.auc",
                "norm"="no", "output"="Act_unfixed_IRF2")
}
...
if(is.na(numtest)){ 
  ...
  rm(ctrl)
  invis <- gc(verbose=FALSE)
  ## Normalize the target to the control
  if(argsL$norm=="yes"){  
  ...
  pctremain<-function(x) {
    # calculate number of peaks above a given quantile in the expAUC and the 
    # combined [expAUC,ctrlAUC] vectors and return the ratio between Exp/(Exp,Ctrl)
    i <- length(expvec) - (ecdf(expvec)(x) * length(expvec))  
    j <- length(both) - (ecdf(both)(x) * length(both))
    i/j
  }
  
  # For every AUC value in the EXP&CTRL, find the EXP to EXP&CTRL ratio for the 
  # number of peaks that are above the given AUC value
  ## x0 = AUC for max ratio of #ofpeaks EXP/EXP&CTRL 
  ## x0 is maxed when 
  x<-sort(unique(both)) ## New for SEACR_1.1
  pctr_x <- na.omit(pctremain(x[pctremain(x) < 1]))
  x0<-x[which(pctr_x == max(pctr_x))]  # AUC for max ratio of #ofpeaks EXP/EXP&CTRL 
  
  # For all AUCs less than x0, calculate the difference between the AUC and the 
  # average PeakRatio
  ## z2 = AUC for average #ofPeaks for EXP and EXP&CTRL
  z<-x[x <= x0[1]]	## New for SEACR_1.1
  diff_avg_z <- abs(((pctremain(x0)+min(pctremain(z)))/2)-pctremain(z))
  z2<-z[diff_avg_z==min(diff_avg_z)] # AUC corresponding to the average #ofPeaks for EXP and EXP&CTRL
  
  setNames(data.frame(cbind(quantile(expvec, seq(0, 1, by =0.01)),
                            quantile(ctrlvec, seq(0, 1, by =0.01)),
                            quantile(both, seq(0, 1, by =0.01)))),
           c('exp', 'ctrl', 'both'))
  
  # Set z0 for minDiff
  if(x0[1]!=z2[1]){ 
    # if maxDiff does not equal the avgDiff
    z<-z[z > z2[1]] # AUCs between average and maxDiff
    mid_z_range <- (1/2)*(max(z)-min(z))
    diff_to_mid_z <- abs(z-(max(z)-mid_z_range)) # |z - midpoint(z)|
    z0<-z[diff_to_mid_z==min(diff_to_mid_z)] # Find AUC for z-point closest to the midpoint of z
  }else{  
    z0<-x0 # set AUC for z-point closest tot he midpoint as the AUC for max ratio of #ofpeaks EXP/EXP&CTRL 
  }
  
  ## Visualize
  if(visualize){
    scale.density <- function(d){
      x <- density(d, n=10000)
      x$y <- scales::rescale(x$y, to=c(0,1))
      return(x)
    }
    pdf("~/xfer/test.pdf")
    plot(scale.density(both), xlim=c(0, quantile(both, 0.99)))
    lines(scale.density(expvec), col='red')
    lines(scale.density(ctrlvec), col='blue')
    abline(v = c(x0, z2, z0), col=c("red", "blue", "grey"))
    legend("topright", fill=c("red", "blue", "black"), 
           legend=c("exp", "ctrl", "both"))
    dev.off()
  }
